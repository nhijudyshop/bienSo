...
export default function DanhSachBienPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-center mb-6">DANH SÁCH CHÍNH THỨC</h1>
      <p className="text-text-secondary text-center text-sm mb-6">
        Biển số xe đã được công bố chính thức, đang trong giai đoạn đăng ký đấu giá
      </p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-text-secondary">Đang tải dữ liệu...</td>
                </tr>
              ) : plates.length === 0 ? (
                <tr>
                ...
                plates.map((item, idx) => (
                  <tr key={item.bksId} className="border-b border-border hover:bg-bg-card transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <a href="https://dgbs.vpa.com.vn" target="_blank" rel="noopener noreferrer"
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2 py-4 border-t border-border">
            <button onClick={() => goToPage(Math.max(0, page - 1))} disabled={page === 0}
              className="px-3 py-1.5 rounded-md text-sm border border-border disabled:opacity-40 hover:bg-bg-card">&lsaquo;</button>
            ...
            <button onClick={() => goToPage(Math.min(totalPages - 1, page + 1))} disabled={page >= totalPages - 1}
              className="px-3 py-1.5 rounded-md text-sm border border-border disabled:opacity-40 hover:bg-bg-card">&rsaquo;</button>
          </div>
        )}
      </div>
